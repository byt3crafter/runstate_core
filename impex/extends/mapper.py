            ...
            if table_map:
                for source_d in source_doc.get(df.fieldname):
                    if "condition" in table_map:
                        if not table_map["condition"](source_d, source_doc):
                            continue
                    ...
                    if (
                        ...
                        and source_d.name
                        not in frappe.flags.selected_children[df.fieldname]
                    ):
                        continue
    ...
    if table_map.get("validation"):
        for key, condition in table_map["validation"].items():
            if condition[0] == "=" and source_doc.get(key) != condition[1]:
                frappe.throw(
                    _("Cannot map because following condition fails:")
                    + f" {key}={cstr(condition[1])}"
                )
            ...
            if not linked_doc:
                try:
                    linked_doc = frappe.get_doc(
                        df.options, target_doc.get(df.fieldname)
                    )
                except Exception:
                    return
